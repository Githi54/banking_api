"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Bank = void 0;
const swagger_1 = require("@nestjs/swagger");
const typeorm_1 = require("typeorm");
let Bank = class Bank {
};
__decorate([
    (0, swagger_1.ApiProperty)({ example: 1, description: 'Unique identificator' }),
    (0, typeorm_1.PrimaryGeneratedColumn)(),
    __metadata("design:type", Number)
], Bank.prototype, "id", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'Mono bank', description: 'Bank name' }),
    (0, typeorm_1.Column)({ nullable: false, default: '', unique: true }),
    __metadata("design:type", String)
], Bank.prototype, "name", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 175, description: 'Unique identificator' }),
    (0, typeorm_1.Column)({ default: 0, nullable: false }),
    __metadata("design:type", Number)
], Bank.prototype, "balance", void 0);
__decorate([
    (0, typeorm_1.CreateDateColumn)({ type: 'timestamp', select: false }),
    __metadata("design:type", Date)
], Bank.prototype, "createdAt", void 0);
__decorate([
    (0, typeorm_1.UpdateDateColumn)({ type: 'timestamp', select: false }),
    __metadata("design:type", Date)
], Bank.prototype, "updatedAt", void 0);
Bank = __decorate([
    (0, typeorm_1.Entity)({ name: 'Bank' })
], Bank);
exports.Bank = Bank;
//# sourceMappingURL=bank.entity.js.map